...
export const EditDishWithIngredients = () => {
  ...
  const [category, setCategory] = useState<"breakfast" | "mainDish" | "snack" | undefined>(undefined);
  const [recipe, setRecipe] = useState("");
  const [ingredients, setIngredients] = useState<DishIngredient[]>([]);
  const [searchQuery, setSearchQuery] = useState("");
  const [showDropdown, setShowDropdown] = useState(false);
  const [aiSearchResult, setAiSearchResult] = useState<SearchableProduct | null>(null);
  ...
  useEffect(() => {
    if (id && dishes.length > 0) {
      ...
      if (dish && dish.isIngredientBased && dish.ingredients) {
        setDishName(dish.name);
        setCategory(dish.category);
        setRecipe(dish.recipe || "");
        setIngredients(dish.ingredients);
      } else if (dish && !dish.isIngredientBased) {
        // Fallback: Falls Gericht nicht ingredient-based ist, redirect zum normalen Editor
        navigate(`/dishes/${id}/edit`, { replace: true });
      }
    }
  }, [id, dishes, navigate]);
  ...
  if (!id) {
    return (
      <div className="card p-8">
        <p className="text-center text-destructive">Fehler: Keine Gericht-ID gefunden</p>
      </div>
    );
  }
      ...
      <div className="card p-4 sm:p-6 md:p-8">
        <div className="mb-6 sm:mb-8">
          ...
          <p className="text-center text-muted-foreground mt-2 text-sm sm:text-base">
            Bearbeite die Zutaten und Nährwerte des Gerichts
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            ...
            <div className="relative">
              ...
              <input
                ...
                onFocus={() => {
                  ...
                  if (aiSearchResult || isAISearching || searchQuery.length > 0) {
                    setShowDropdown(true);
                  }
                }}
                onBlur={() => {
                  ...
                  setTimeout(() => {
                    ...
                    if (!activeElement || !activeElement.closest(".search-container")) {
                      setShowDropdown(false);
                    }
                  }, 300);
                }}
                className={`input w-full ${hasGeminiAPIKey ? 'pl-11 sm:pl-10' : ''} text-base`}
                placeholder="z.B. Banane, Milch, Brot..."
              />
            </div>

            {/* Dropdown mit Suchergebnissen */}
            {showDropdown && (aiSearchResult || isAISearching) && (
              <div 
                    ...
                    {aiSearchResult && (
                      ...
                      >
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span className="font-medium text-sm sm:text-base break-words">{aiSearchResult.name}</span>
                        ...
                        <div className="text-xs sm:text-sm text-muted-foreground mt-1 break-words">
                          {aiSearchResult.nutritionUnit}: {Math.round(aiSearchResult.caloriesPerUnit)} kcal,{" "}
            ...
            {ingredients.length === 0 ? (
              ...
              <div className="space-y-2">
                {ingredients.map((ingredient, index) => {
                  ...
                  const calculatedCalories = ingredient.caloriesPerUnit * factor;
                  ...
                  return (
                    ...
                    >
                      <div className="flex-1 w-full sm:w-auto">
                        <div className="font-medium mb-1 text-sm sm:text-base break-words">{ingredient.name}</div>
                        <div className="text-xs sm:text-sm text-muted-foreground break-words">
                          {Math.round(calculatedCalories)} kcal, {Math.round(calculatedProtein)}g Protein,{" "}
                          {Math.round(calculatedCarbs)}g Carbs, {Math.round(calculatedFat)}g Fett
                          <span className="ml-1 sm:ml-2 text-xs">
                            ({ingredient.nutritionUnit}: {Math.round(ingredient.caloriesPerUnit)} kcal)
                          </span>
                        </div>
                      ...
                      <div className="flex items-center gap-2 w-full sm:w-auto justify-end sm:justify-start">
                        ...
                        />
                        <span className="text-sm text-muted-foreground w-8 sm:w-8 flex-shrink-0">{ingredient.unit}</span>
                        <button
                          type="button"
          ...
          {ingredients.length > 0 && (
            <div className="p-3 sm:p-4 bg-primary/10 border border-primary/20 rounded-lg">
              <h3 className="font-semibold mb-3 text-base sm:text-lg">Gesamtnährwerte</h3>
              <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
                <div>
                  <div className="text-xs sm:text-sm text-muted-foreground">Kalorien</div>
          ...
          <div>
            <label htmlFor="dishName" className="block text-sm font-medium mb-2">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setCategory(
                  e.target.value as "breakfast" | "mainDish" | "snack" | undefined
                )
              }
              className="input w-full"
              required
          ...
          <div className="flex flex-col sm:flex-row justify-end gap-3 sm:gap-4 pt-4">
            <Link to="/dishes" className="btn-secondary w-full sm:w-auto text-center touch-manipulation min-h-[44px] flex items-center justify-center">
              Abbrechen
            </Link>
            <button
              type="submit"
